...
sheet_to_analyze <- "AvailabilityBiasQ1_CLAUDE"
...
analysis_data <- data_long |>
  mutate(
    Source = case_when(
      Source_Original == eng_col_name  ~ "EnglishCLAUDE",
      Source_Original == heb_col_name    ~ "HebrewCLAUDE",
      Source_Original == rus_col_name ~ "RussianCLAUDE"
    ),
    Is_Correct_Binary = ifelse(Correctness == TRUE, 1, 0),
    Source = factor(Source, levels = c("EnglishCLAUDE", "HebrewCLAUDE", "RussianCLAUDE"))
  )
