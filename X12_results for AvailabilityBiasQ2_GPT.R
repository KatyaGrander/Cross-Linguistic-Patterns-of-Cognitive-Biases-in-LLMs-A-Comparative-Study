...
sheet_to_analyze <- "AvailabilityBiasQ2_GPT"
...
analysis_data <- data_long |>
  mutate(
    Source = case_when(
      Source_Original == eng_col_name  ~ "EnglishGPT",
      Source_Original == heb_col_name    ~ "HebrewGPT",
      Source_Original == rus_col_name ~ "RussianGPT"
    ),
    Is_Correct_Binary = ifelse(Correctness == TRUE, 1, 0),
    Source = factor(Source, levels = c("EnglishGPT", "HebrewGPT", "RussianGPT"))
  )
